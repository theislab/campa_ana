...
annotation <- bind_cols(cluster_id=seq(0,20),color=res1_2_colors)
...
cell_aggregate_data <- selected_wells %>%
  mutate(model_res = map(campa_res_dir,~read_csv(file.path(.,"export",paste0("intensity_features_clustering_res",sprintf("%0.1f",current_resolution),".csv")),show_col_types = FALSE) %>% select(-1))) %>%
  select(-well_name) %>%
  unnest(model_res)
...
pixels <- selected_wells %>%
  mutate(
    ...
    leiden_cluster_id = map(campa_res_dir,~load_pixel_clustering(.,paste0("clustering_res",sprintf("%0.1f",current_resolution),".npy")))) 
...
segmentation_images <- selected_pixels_outlines %>%
  ...
  mutate(cluster_id=factor(as.character(cluster_id),levels=as.character(seq(0,19)))) %>%
  
  ungroup() %>% {
    ggplot(data=.,aes(x=x,y=y,fill=cluster_id)) + 
      geom_tile() + 
      ...
      scale_fill_manual(values = annotation$color[match(levels(as.factor(selected_pixels_outlines$cluster_id)),annotation$cluster_id)])
  }
segmentation_images
ggsave(segmentation_images,file=file.path(plot_dir,paste0("tsa_versus_control_examples_full_segmentation_direct_res",sprintf("%0.1f",current_resolution),".pdf")),width=16,height=3.5,units="cm")
ggsave_cairo(segmentation_images,file=file.path(plot_dir,paste0("tsa_versus_control_examples_full_segmentation_direct_res",sprintf("%0.1f",current_resolution),".png")),width=16,height=3.5,units="cm",dpi=600)
...
legend_plot <- ggpubr::get_legend(p) %>%
  ggpubr::as_ggplot()
legend_plot
ggsave(legend_plot,file=file.path(plot_dir,paste0("184A1_annotation_legend_direct_res",sprintf("%0.1f",current_resolution),".pdf")),width=6.5,height=2.5,units="cm")
ggsave_cairo(legend_plot,file=file.path(plot_dir,paste0("184A1_annotation_legend_direct_res",sprintf("%0.1f",current_resolution),".png")),width=2.5,height=2.5,units="cm",dpi=600)

# plot channel images

channel_image <- selected_pixels_outlines %>%
  filter(mapobject_id %in% distinct(selected_pixels_outlines,mapobject_id)$mapobject_id[1:8]) %>%
  select(mapobject_id,treatment,y,x,outline,`10_H3K27ac`) %>%
  ungroup() %>%
  group_by(mapobject_id) %>%
  mutate(x=x-min(x),y=y-min(y)) %>%
  ungroup() %>%
  mutate(`10_H3K27ac` = `10_H3K27ac` / quantile(`10_H3K27ac`,0.975)) %>% {
    ggplot(data=.,aes(x=x,y=y,fill=`10_H3K27ac`)) + 
      geom_tile() +
...
ggsave(channel_image,file=file.path(plot_dir,paste0("tsa_versus_control_examples_H3K27ac_direct_res",sprintf("%0.1f",current_resolution),".pdf")),width=16,height=3.5,units="cm")
ggsave_cairo(channel_image,file=file.path(plot_dir,paste0("tsa_versus_control_examples_H3K27ac_direct_res",sprintf("%0.1f",current_resolution),".png")),width=16,height=3.5,units="cm",dpi=600)
