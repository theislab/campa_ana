...
cx5461_cell <- 209370
dmso_cell <- 233649
example_mapobject_ids <- c(cx5461_cell,dmso_cell)
...
segmentation_images
ggsave(segmentation_images,file=file.path(plot_dir,"cx5461_versus_control_examples_full_segmentation.pdf"),width=6,height=2.5,units="cm")
ggsave_cairo(segmentation_images,file=file.path(plot_dir,"cx5461_versus_control_examples_full_segmentation.png"),width=6,height=2.5,units="cm",dpi=600)
...
selected_pixels_nucleolus_outlines <- selected_pixels %>%
  ungroup() %>%
  filter(cluster_annotation=="Nucleolus") %>%
  group_by(mapobject_id) %>%
  nest() %>%
  mutate(sparse = map(data,to_sparse)) %>%
  #mutate(sparse = map(sparse,~erode_sparse(.,width=1))) %>% 
  #mutate(sparse = map(sparse,~dilate_sparse(.,width=1))) %>% 
...
plot_single_channel <- function(dat,channel_name,percentile_rescaling=0.997,write_out=T) {
  ...
  channel_image <- dat %>%
    ...
    mutate(value = value / quantile(value,percentile_rescaling)) %>% {
      ggplot(data=.,aes(x=x,y=y,fill=value)) + 
        ...
        scale_fill_viridis_c(name="Intensity",option ="D",limits=c(0,1),oob=scales::oob_squish,breaks=c(0,0.5,1)) +
        theme_void(base_size = 7) +
        coord_fixed() + 
        theme(legend.position = "none",
  ...
  if (write_out) {
    ggsave(channel_image,file=file.path(plot_dir,paste0("cx5461_versus_control_examples_channel_images_",rlang::as_name(channel_name),".pdf")),width=6,height=2.5,units="cm")
    ggsave_cairo(channel_image,file=file.path(plot_dir,paste0("cx5461_versus_control_examples_channel_images_",rlang::as_name(channel_name),".png")),width=6,height=2.5,units="cm",dpi=600)
  }
...
p <- plot_single_channel(selected_pixels_nucleolus_outlines,`21_NCL`,percentile_rescaling = 0.995) +
  theme(legend.position = "right",
        legend.key.size=unit(3,"mm"))
