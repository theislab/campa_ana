...
selected_wells <- wells_metadata %>%
  filter(cell_type=="184A1" & treatment=="Unperturbed") %>%
  filter(EU==30) %>%
...
not_identified <- campa_res %>%
  filter(size==0) %>%
  select(cell_cycle,mapobject_id,cluster)

# summarise these
fraction_not_identified <- campa_res %>%
  mutate(zero_size = size==0) %>%
  group_by(cell_cycle,cluster,zero_size) %>%
  count() %>%
  group_by(cell_cycle,cluster) %>%
  mutate(frac_identified = n/sum(n)) %>%
  filter(zero_size == FALSE)

ggplot(fraction_not_identified,aes(x=cluster,y=frac_identified,fill=cell_cycle)) + 
...
non_positive_intensity <- campa_res %>%
  ...
  filter(intensity <=0) %>%
  select(cell_cycle,mapobject_id,cluster,channel)
...
fraction_non_positive_intensity <- campa_res %>%
  ...
  group_by(cell_cycle,cluster,non_positive_intensity) %>%
  count() %>%
  group_by(cell_cycle,cluster) %>%
  mutate(frac_positive_intensity = n/sum(n)) %>%
  filter(non_positive_intensity == FALSE)

ggplot(fraction_non_positive_intensity,aes(x=cluster,y=frac_positive_intensity,fill=cell_cycle)) + 
  geom_col(position = "dodge")
...
# compute intensity fold-changes across all cell cycle stages and channels separately ----
  ...
  for (current_cell_cycle in all_cell_cycle) {
    ...
    intensity_fold_changes_list <- foreach (i=1:length(all_channels)) %dopar% {
      ...
      res <- purrr::map_dfr(
        ...
        ~fit_mixed_model_per_CSL(
          dat = filter(campa_res_long_intensities,
                       cell_cycle %in% c("G1",current_cell_cycle) & cluster %in% c(.,"all")),
          var = intensity,
          channel_name = all_channels[i],
          transform = "log",
          object_id = mapobject_id,
          random_effect = well_name,
          contrast_var = cell_cycle,
          contrast_var_reference = "G1",
          group_var = cluster,
          group_var_reference = "all",
          unnormalised_only = F)
      )
    ...
    write_csv(x = intensity_fold_changes,
              file = file.path(model_dir,paste0("184A1_intensity_fold_changes_",current_cell_cycle,".csv")))
  }
)

# compute size fold-changes across all cell cycle stages separately ----
...
system.time(
  for (current_cell_cycle in all_cell_cycle) {
    ...
    size_fold_changes <- purrr::map_dfr(
      all_clusters,
      ~fit_mixed_model_per_CSL(
        dat = filter(campa_res_sizes,cell_cycle %in% c("G1",current_cell_cycle) & cluster %in% c(.,"all")),
        var = size,
        transform = "log",
        object_id = mapobject_id,
        random_effect = well_name,
    ...
    write_csv(x = size_fold_changes,
              file = file.path(model_dir,paste0("184A1_size_fold_changes_",current_cell_cycle,".csv")))
