...
intensity_fold_changes_full <- map_dfr(
  list.files(model_dir) %>% 
    grepl(pattern = "intensity_fold_changes_(S|G2)\\.",x = .,perl=T) %>%
    list.files(model_dir)[.],
...
size_fold_changes_full <- map_dfr(
  list.files(model_dir) %>% 
    grepl(pattern = "size_fold_changes_(S|G2)\\.",x = .,perl=T) %>%
    list.files(model_dir)[.],
  ~read_csv(file = file.path(model_dir,.))
)
...
fold_changes_to_plot <- intensity_fold_changes_full %>%
  filter(cluster=="all") %>%
  group_by(cluster,channel,cell_cycle) %>%
  filter(p.value==max(p.value)) %>% 
  filter(abs(fold_change)==min(abs((fold_change)))) %>%
  ungroup() %>%
  bind_rows(filter(intensity_fold_changes_full,cluster!="all")) %>%
  mutate(cell_cycle = str_remove(cell_cycle," - G1"),
...
# 34 channels x (6 clusters + overall) x 2 cell-cycle = 476
fold_changes_to_plot %>%
  filter(comparison=="unnormalised") %>%
  nrow()

# (6 clusters + overall) x 2 cell-cycle = 14
size_fold_changes_to_plot %>%
  filter(comparison=="unnormalised") %>%
  nrow()

# 34 channels x 6 clusters x 2 cell-cycle = 408
fold_changes_to_plot %>%
  filter(comparison=="relative_to_all") %>%
  nrow()

# 6 clusters x 2 cell-cycle = 12
size_fold_changes_to_plot %>%
  filter(comparison=="relative_to_all") %>%
  nrow()

# Loop over all cell cycle
all_cell_cycle <- unique(fold_changes_to_plot$cell_cycle)
for (current_cell_cycle in all_cell_cycle) {
  
  # Plot unnormalised
  to_plot <- fold_changes_to_plot %>%
    filter(cell_cycle==current_cell_cycle) %>%
    filter(comparison=="unnormalised")
  ...
  ggsave(plot = combined_plot,filename = file.path(plot_dir,paste0("unnormalised_dot_plot_",current_cell_cycle,".pdf")),width=10,height=5,units="cm")
  ggsave_cairo(plot = combined_plot,filename = file.path(plot_dir,paste0("unnormalised_dot_plot_",current_cell_cycle,".png")),width=10,height=5,units="cm",dpi=600)
  ...
  ggsave(plot = combined_plot,filename = file.path(plot_dir,paste0("normalised_dot_plot_",current_cell_cycle,".pdf")),width=10,height=5,units="cm")
...
whole_nucleus_plot <- make_bubble_plot(fold_changes = to_plot,
                   plot_var = log2_fold_change,
                   col_var = channel_name,
                   row_var = cell_cycle,
                   col_order=cols,
                   row_order=c("G2","S"),
                   color_limits=c(-1,1)) + xlab("Channel")

ggsave(plot = whole_nucleus_plot,filename = file.path(plot_dir,paste0("whole_nucleus_dot_plot_cell_cycle.pdf")),width=10,height=4,units="cm")
ggsave_cairo(plot = whole_nucleus_plot,filename = file.path(plot_dir,paste0("whole_nucleus_dot_plot_cell_cycle.png")),width=10,height=4,units="cm",dpi=600)
