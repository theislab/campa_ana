...
cell_aggregate_data <- selected_wells %>%
  mutate(model_res = map(campa_res_dir,~read_csv(file.path(.,"export","intensity_features_seed3.csv"),show_col_types = FALSE) %>% select(-1))) %>%
...
selected_pixels <- selected_pixels %>%
  select(mapobject_id,x,y,siRNA,well_name,`22_DDX6`,cluster_id,
         cluster_annotation,color,cluster_annotation_original,
         campa_res_dir,data_dir,size,cluster,cell_cycle) %>%
...
compartment <- "P-bodies"
selected_pixels_outlines %>%
  mutate(x=x-min(x),y=y-min(y)) %>%
  ungroup() %>% {
    ggplot(data=.,aes(x=x,y=y,fill=as.character(cluster_id))) + 
...
ggsave(file=file.path(plot_dir,paste0("scrambled_SBF2_examples_P-bodies_segmentation_raw_csl.pdf")),width=12,height=7,units="cm")
ggsave_cairo(file=file.path(plot_dir,paste0("scrambled_SBF2_examples_P-bodies_segmentation_raw_csl.png")),width=12,height=7,units="cm",dpi=600)
...
plot_single_channel(selected_pixels_outlines,`22_DDX6`,percentile_rescaling=0.999)
